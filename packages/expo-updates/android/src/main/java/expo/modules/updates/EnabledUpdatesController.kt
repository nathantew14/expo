...
class EnabledUpdatesController(
  ...
  override fun getConstantsForModule(context: Context): UpdatesModuleConstants {
    return UpdatesModuleConstants(
      ...
      runtimeVersion = updatesConfiguration.runtimeVersion,
  ...
  override fun checkForUpdate(context: Context, callback: IUpdatesController.ModuleCallback<CheckForUpdateResult>) {
    stateMachine.processEvent(UpdatesStateEvent.Check())
      ...
      fileDownloader.downloadRemoteUpdate(
        ...
        object : FileDownloader.RemoteUpdateDownloadCallback {
          override fun onFailure(message: String, e: Exception) {
            ...
            callback.onSuccess(CheckForUpdateResult.ErrorResult(e, message))
          }
          ...
          override fun onSuccess(updateResponse: UpdateResponse) {
            ...
            if (updateDirective != null) {
              if (updateDirective is UpdateDirective.RollBackToEmbeddedUpdateDirective) {
                if (!updatesConfiguration.hasEmbeddedUpdate) {
                  callback.onSuccess(CheckForUpdateResult.NoUpdateAvailable(LoaderTask.RemoteCheckResultNotAvailableReason.ROLLBACK_NO_EMBEDDED))
                ...
                if (!selectionPolicy.shouldLoadRollBackToEmbeddedDirective(
                  ...
                  callback.onSuccess(CheckForUpdateResult.NoUpdateAvailable(LoaderTask.RemoteCheckResultNotAvailableReason.ROLLBACK_REJECTED_BY_SELECTION_POLICY))
                  stateMachine.processEvent(UpdatesStateEvent.CheckCompleteUnavailable())
                  return
                }

                callback.onSuccess(CheckForUpdateResult.RollBackToEmbedded(updateDirective.commitTime))
            ...
            if (updateManifest == null) {
              callback.onSuccess(CheckForUpdateResult.NoUpdateAvailable(LoaderTask.RemoteCheckResultNotAvailableReason.NO_UPDATE_AVAILABLE_ON_SERVER))
            ...
            if (launchedUpdate == null) {
              ...
              callback.onSuccess(CheckForUpdateResult.UpdateAvailable(updateManifest))
            ...
            if (shouldLaunch) {
              ...
              callback.onSuccess(CheckForUpdateResult.NoUpdateAvailable(reason))
              stateMachine.processEvent(UpdatesStateEvent.CheckCompleteUnavailable())
              return
            }
          }
  ...
  override fun fetchUpdate(context: Context, callback: IUpdatesController.ModuleCallback<FetchUpdateResult>) {
    stateMachine.processEvent(UpdatesStateEvent.Download())
      ...
      RemoteLoader(
        ...
        .start(
          object : Loader.LoaderCallback {
            override fun onFailure(e: Exception) {
              databaseHolder.releaseDatabase()
              callback.onSuccess(FetchUpdateResult.ErrorResult(e))
              stateMachine.processEvent(
            ...
            override fun onSuccess(loaderResult: Loader.LoaderResult) {
              RemoteLoader.processSuccessLoaderResult(
                ...
                if (didRollBackToEmbedded) {
                  callback.onSuccess(FetchUpdateResult.RollBackToEmbedded())
                  stateMachine.processEvent(UpdatesStateEvent.DownloadCompleteWithRollback())
                } else {
                  if (availableUpdate == null) {
                    callback.onSuccess(FetchUpdateResult.Failure())
                    stateMachine.processEvent(UpdatesStateEvent.DownloadComplete())
                  } else {
                    ...
                    callback.onSuccess(FetchUpdateResult.Success(updateEntity))
